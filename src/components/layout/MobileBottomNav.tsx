...
function isActive(pathname: string, key: NavKey) {
  ...
  if (key === "series") return pathname.startsWith("/series");
  if (key === "search") return pathname.startsWith("/search");
  return ["/about", "/contact", "/privacy", "/terms"].some((path) => pathname.startsWith(path));
}
...
function NavIcon({ name }: { name: NavKey }) {
  const paths: Record<NavKey, React.ReactNode> = {
    ...
    info: <><circle cx="12" cy="12" r="8.5"/><path d="M12 10.8V17"/><path d="M12 7.5h.01"/></>,
  };
    ...
    <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-white/10 bg-[#0b0b0f]/95 pb-[max(env(safe-area-inset-bottom),0.35rem)] pt-1.5 backdrop-blur-xl sm:hidden" aria-label="Navigasi utama mobile">
      <div className="mx-auto grid max-w-md grid-cols-4 px-2">
        {items.map((item) => {
          ...
          return (
            <Link key={item.key} href={item.href} aria-current={active ? "page" : undefined} className={`flex min-h-14 flex-col items-center justify-center gap-1 rounded-2xl text-[10px] font-bold transition active:scale-95 ${active ? "text-white" : "text-zinc-500"}`}>
