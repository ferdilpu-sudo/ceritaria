...
function NavIcon({ name }: { name: IconName }) {
  if (name === "home") {
    return <path d="M3 10.7 12 3l9 7.7V21a1 1 0 0 1-1 1h-5v-7H9v7H4a1 1 0 0 1-1-1V10.7Z" />;
  }
  if (name === "series") {
    return <><rect x="4" y="3" width="16" height="18" rx="2" /><path d="M8 8h8M8 12h8M8 16h5" /></>;
  }
  if (name === "episode") {
    return <><rect x="3" y="5" width="18" height="14" rx="2" /><path d="m10 9 5 3-5 3V9Z" /></>;
  }
...
export function AdminNav() {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 border-b border-[var(--border)] bg-white/95 backdrop-blur-xl sm:hidden">
        <div className="flex min-h-14 items-center justify-between gap-3 px-4">
          <Link href="/admin" className="text-[13px] font-black tracking-[0.17em] text-[var(--text)]">
          ...
          <div className="flex items-center gap-1">
            ...
            <form action={logoutAction}>
              <button
                ...
                className="grid h-11 w-11 place-items-center rounded-full text-[var(--muted)] active:bg-red-50 active:text-red-700"
              >
                <svg aria-hidden="true" viewBox="0 0 24 24" className="h-5 w-5 fill-none stroke-current" strokeWidth="1.8">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-[var(--border)] bg-white/95 pb-[env(safe-area-inset-bottom)] shadow-[0_-8px_24px_rgba(15,23,42,0.08)] backdrop-blur-xl sm:hidden" aria-label="Navigasi admin mobile">
        <div className="grid grid-cols-4">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} data-guide={item.guide} aria-current={active ? "page" : undefined}
                className={`relative flex min-h-16 flex-col items-center justify-center gap-1 px-1 text-[10px] font-bold transition ${active ? "text-red-600" : "text-[var(--muted)] active:bg-[var(--surface-2)]"}`}>
