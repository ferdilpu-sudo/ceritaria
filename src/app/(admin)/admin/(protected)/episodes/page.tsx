...
export default async function AdminEpisodesPage({
  ...
  const [{ saved, view }, episodes, series] = await Promise.all([searchParams, getAdminEpisodes(), getAdminSeries()]);
  ...
  return (
    <div className="min-w-0">
      <AdminSavedNotice kind="Episode" state={saved} publicPath={view} />
      <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-2 break-words text-sm text-[var(--muted)]">
            {episodes.length} episode · {published} tayang · {episodes.length - published} draft
          </p>
        </div>
      ...
      <div className="mt-7 min-w-0 space-y-3">
        {episodes.length ? (
                ...
                <div className="flex min-w-0 items-start gap-3 sm:flex-1 sm:items-center sm:gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex min-w-0 flex-wrap items-center gap-2">
                      ...
                      <span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-black ${providerChip(provider)}`}>
                        {provider === "YOUTUBE" ? "YouTube" : provider === "FACEBOOK" ? "Facebook" : provider}
                      </span>
                    </div>
          ...
          <div className="surface min-w-0 rounded-2xl p-7 text-center sm:p-9">
            <div className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-red-50 text-xl font-black text-red-600">▶</div>
            <p className="mt-4 font-black text-[var(--text)]">Belum ada episode</p>
            <p className="mx-auto mt-2 max-w-sm break-words text-sm leading-6 text-[var(--muted)]">
              {series.length ? "Tambahkan video pertama ke series yang sudah kamu buat." : "Buat series terlebih dahulu, lalu tambahkan episode videonya."}
            </p>
            <Link href={series.length ? "/admin/episodes/new" : "/admin/series/new"} className="mt-5 inline-flex min-h-11 items-center rounded-xl bg-[var(--primary)] px-4 py-3 text-sm font-black text-white hover:bg-[var(--primary-hover)]">
              {series.length ? "Buat Episode Pertama" : "Buat Series Dulu"}
            </Link>
          </div>
        )}
      </div>
