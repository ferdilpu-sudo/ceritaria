...
export default async function AdminSeriesPage({
  ...
  const [{ saved, view }, series] = await Promise.all([searchParams, getAdminSeries()]);
  const published = series.filter((item) => item.is_published).length;

  return (
    <div className="min-w-0">
      <AdminSavedNotice kind="Series" state={saved} publicPath={view} />
      <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-2 break-words text-sm text-[var(--muted)]">
            {series.length} series · {published} tayang · {series.length - published} draft
          </p>
        </div>
      ...
      <div className="mt-7 min-w-0 space-y-3">
        {series.length ? (
              ...
              <div className="flex min-w-0 items-start gap-3 sm:flex-1 sm:items-center sm:gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex min-w-0 flex-wrap items-center gap-2">
                    ...
                    {item.is_featured && <span className="shrink-0 rounded-full border border-rose-200 bg-rose-50 px-2.5 py-1 text-[10px] font-black text-rose-700">UNGGULAN</span>}
                  </div>
          ...
          <div className="surface min-w-0 rounded-2xl p-7 text-center sm:p-9">
            ...
            <p className="mt-4 font-black text-[var(--text)]">Belum ada series</p>
            <p className="mx-auto mt-2 max-w-sm break-words text-sm leading-6 text-[var(--muted)]">Buat series pertama sebagai tempat untuk mengumpulkan episode ceritamu.</p>
            <Link href="/admin/series/new" className="mt-5 inline-flex min-h-11 items-center rounded-xl bg-[var(--primary)] px-4 py-3 text-sm font-black text-white hover:bg-[var(--primary-hover)]">
              Buat Series Pertama
            </Link>
          </div>
        )}
