...
async function getEpisodeContext(slug: string, episodeSlug: string) {
  ...
  return { series, episode, allEpisodes, previous: index > 0 ? allEpisodes[index - 1] : undefined, next: index >= 0 ? allEpisodes[index + 1] : undefined };
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: episode.title, description: episode.short_synopsis ?? undefined, images: episode.thumbnail_url ? [episode.thumbnail_url] : undefined },
  };
}

export default async function EpisodePage({ params, searchParams }: PageProps) {
  const [{ slug, episodeSlug }, query] = await Promise.all([params, searchParams]);
  const context = await getEpisodeContext(slug, episodeSlug);
  if (!context) notFound();
  ...
  const jsonLd = {
    ...
    partOfSeries: { "@type": "TVSeries", name: series.title, url: `${NEXT_PUBLIC_SITE_URL}/series/${series.slug}` },
    url: `${NEXT_PUBLIC_SITE_URL}/series/${series.slug}/${episode.slug}`, description: episode.short_synopsis ?? undefined,
  };
  const nextHref = next ? `/series/${series.slug}/${next.slug}` : undefined;

  return (
    <div className="shell pb-0 pt-2 sm:pt-9">
      <EpisodeViewTracker episodeId={episode.id} />
      <ContinueWatchingTracker episodeId={episode.id} seriesSlug={series.slug} seriesTitle={series.title} episodeSlug={episode.slug} episodeTitle={episode.title} episodeNumber={episode.episode_number} thumbnailUrl={episode.thumbnail_url} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replace(/</g, "\\u003c") }} />

      <nav aria-label="Kembali ke series" className="mb-2 sm:hidden"><Link href={`/series/${series.slug}`} className="inline-flex min-h-11 max-w-full items-center truncate rounded-2xl pr-4 text-sm font-bold text-zinc-300 active:scale-[0.98]">← {series.title}</Link></nav>
