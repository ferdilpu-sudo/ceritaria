...
export function ContinueWatchingSection() {
  ...
  const visible = (unfinished.length ? unfinished : items).slice(0, 6);
  ...
  return (
    <section id="lanjut-nonton" className="mt-8 scroll-mt-20 sm:mt-14 sm:scroll-mt-24" aria-labelledby="continue-watching-title">
      <div className="mb-4 flex items-end justify-between gap-4 sm:mb-5">
        <div>
          <p className="text-[10px] font-black tracking-[0.18em] text-red-400 sm:text-xs">UNTUK KAMU</p>
          <h2 id="continue-watching-title" className="mt-1 text-[22px] font-black sm:text-2xl">Lanjutkan Menonton</h2>
        </div>
        <Link href="/lanjut" className="min-h-11 rounded-lg px-2 py-3 text-sm font-bold text-zinc-400 hover:text-white">Riwayat →</Link>
      </div>
      <div className="mobile-scrollbar -mx-4 flex snap-x snap-mandatory gap-3 overflow-x-auto px-4 pb-3 sm:mx-0 sm:gap-4 sm:px-0">
        {visible.map((item) => <WatchHistoryCard key={`${item.seriesSlug}/${item.episodeSlug}`} item={item} />)}
      </div>
    </section>
  );
}
